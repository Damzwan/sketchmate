<template>
  ...
  >
    <!--
      @touchmove.stop on the OUTER wrapper prevents Ionic's sheet-drag gesture
      from firing on header / preview / list. The handle bar at the top of the
      modal still gets touch events because it lives outside this wrapper.
    -->
    <div
      @touchmove.stop
      class="h-full flex flex-col p-5 bot-pad-safe bg-background cabin-sketch-regular overflow-hidden"
    >
      <!-- Header -->
      ...
      >
        ...
        <div v-else class="space-y-3 mt-2">
          ...
          <button
            ...
            :key="friend._id"
            :disabled="isFriendDisabled(friend) || sendingTo !== null"
            @click="!isFriendDisabled(friend) && selectFriend(friend)"
            :class="[
              ...
              isFriendDisabled(friend)
                ...
                : sendingTo && sendingTo !== friend._id
          ...
          >
            ...
            <div
              v-if="!isFriendDisabled(friend)"
              class="w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 transition-all border-black/5 bg-black/5 text-black/40"
            >
              <ion-spinner
                v-if="sendingTo === friend._id"
                name="dots"
                class="w-4 h-4"
              />
              <ion-icon
                v-else
                :icon="svg(mdiSendOutline)"
                class="text-sm transform -rotate-12 ml-0.5"
              />
            </div>
          </button>
        </div>
...
const selectFriend = async (friend: any) => {
	if (!post.value || sendingTo.value) return;
	sendingTo.value = friend._id;

	const existingChat = chatStore.activeChats.find((c) =>
		c.participants.some((p: any) => p._id === friend._id),
	);
	const tabId = existingChat?._id || friend._id;
	...
	try {
		await chatStore.sendMessage(
			...
			tabId,
			post.value._id,
			{ silent: true }, // ← don't yank the widget into this conversation
		);
		toast(`Shared with ${friend.name}`, { color: "success" });
		handleDismiss();
	} catch (e: any) {
		toast(e?.message || "Failed to share", { color: "danger" });
	} finally {
...
const handleDismiss = () => {
	...
	// Clear the active share post after the dismiss animation completes
...
/* overscroll-contain stops the list's bounce from being interpreted by the
   parent (Ionic modal) as a sheet drag when the list is at top/bottom. */
.overscroll-contain {
  overscroll-behavior: contain;
}

/* Scoped to the scroll container so we don't accidentally mask any other
   .overflow-y-auto we add later. */
.scroll-mask {
