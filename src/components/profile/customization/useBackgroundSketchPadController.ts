...
import {
	...
	type Stroke,
	simplifyPath,
} from "./backgroundSketchGeometry";
import { useStrokeHistory } from "./useStrokeHistory";

export type BackgroundSketchPadProps = {
	isOpen: boolean;
	color?: string;
...
type BackgroundSketchPadEmit = {
	(event: "close"): void;
	(event: "save", payload: { path: string; viewBox: string }): void;
};

export function useBackgroundSketchPadController(
	props: BackgroundSketchPadProps,
	emit: BackgroundSketchPadEmit,
) {
