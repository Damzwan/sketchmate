...
export function useGallerySelection(user: Ref<User | undefined>, inbox: Ref<InboxItem[]>, openPhotoSwiper: (item: any) => void) {
  const api = useAPI()
  ...
  async function handleShare() {
    const itemsToShare = inbox.value.filter((i: any) => selectedItems.value.includes(i._id))
    ...
    try {
      await shareImages(imageUrls, 'Check out my sketches!', 'Share with friends')
    } catch (e) {
