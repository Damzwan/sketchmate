...
function removeStrokeFromClip(
	...
	const baked = (canvasObj as any).__bakedClipStrokes as
		| fabric.Object[]
		| undefined;
...
export async function handleErasedAction(
	...
	if (actionType === "redo") {
		...
		if (rect) {
			...
			if (
				canStampRedo(mgr, enlivenedStroke, footprint, objectIds, deletedIds)
			) {
...
function canStampRedo(
	...
	if ((stroke as any).globalCompositeOperation !== "destination-out")
		return false;
...
function strokeFootprint(
	stroke: fabric.Object | null | undefined,
): WorldRect | null {
