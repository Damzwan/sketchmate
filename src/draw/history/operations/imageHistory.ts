...
export async function redoImgFilter(
	...
	if (action.params.prevFilter) {
		if (action.params.prevFilter.type == "BlendColor")
			img.filters = img.filters?.filter((f: any) => f.type != "BlendColor");
		const [filterAlive] = await fabric.util.enlivenObjects<any>([
			action.params.prevFilter,
		]); // TODO mismatch between undo/redo sync (json instaed of object) and normal
...
export async function undoImgFilter(
	...
	if (prevFilter) {
		...
		if (prevBlendColorFilter) {
			const [prevBlendColorFilterAlive] = await fabric.util.enlivenObjects<any>(
				[prevBlendColorFilter],
			); // TODO mismatch between undo/redo sync (json instaed of object) and normal
