...
import type { FabricEvent } from '@/draw/types/draw.types'
...
export const useClaimArea = defineStore('claimArea', () => {
  ...
  const canClaimMore = computed(() => myAreas.value.length < MAX_AREAS_PER_USER)
  ...
  // ── enforcement API (read by fabricDefaults overrides + eraser) ────────────
  ...
  function rejectMoveIfProtected(): boolean {
    ...
    const snapshot = active.map((o) => ({ o, orig: originals.get((o as any).id) }))
  ...
  function enterClaimMode() {
    ...
    if (!canClaimMore.value) {
      useToast().toast(
        `You can claim at most ${MAX_AREAS_PER_USER} areas`,
        { color: 'warning' }
      )
      return
    }
  ...
  function releaseArea(id: string) {
    ...
    socket?.emit('release-area', { roomId: useDrawSyncer().roomId, areaId: id })
  }
  ...
  function releaseMine() {
    ...
    for (const a of mine)
      socket?.emit('release-area', { roomId: useDrawSyncer().roomId, areaId: a.id })
