...
/**
 * Build the automatic tooldock thumbnail from the worker-prepared drag bitmap.
 * This is deliberately separate from bakeThumbnail(): opening or sharing the
 * large preview may render on demand, but selecting hundreds of objects must
 * never do that work in an animation frame.
 */
export async function bakeCachedThumbnail(
	c: Canvas,
	maxPx = 96,
	objects?: FabricObject[],
): Promise<{ bitmap: ImageBitmap; cssW: number; cssH: number } | null> {
	...
	if (
		...
		(objects?.length &&
			!sameSet(
				isActiveSelection(target) ? [...(target as any)._objects] : [target],
				objects,
			)) ||
		!cachedBakeMatches(target, c.viewportTransform![0], false) ||
		!cachedBake
	) {
		return null;
	}
	...
	const startedAt = performance.now();
	const { width, height } = cachedBake.origin;
	const fit = maxPx / Math.max(width, height);
	const cssW = Math.max(1, Math.round(width * fit));
	const cssH = Math.max(1, Math.round(height * fit));
	const dpr = getRenderDpr();
	const physW = Math.max(1, Math.round(cssW * dpr));
	const physH = Math.max(1, Math.round(cssH * dpr));
	const off = new OffscreenCanvas(physW, physH);
	...
	try {
		ctx.drawImage(cachedBake.bitmap, 0, 0, physW, physH);
		const bitmap = off.transferToImageBitmap();
		recordPhase("selectionThumbnail", performance.now() - startedAt);
		return { bitmap, cssW, cssH };
	} catch {
		return null;
	}
}
