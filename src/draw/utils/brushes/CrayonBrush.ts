...
export function generateCrayonImage(
  ...
  } catch { /* setTransform unsupported → tiny phase diff, invisible in noise */ }
...
export class CrayonStroke extends FabricImage {
  ...
  static cacheProperties = [
    ...
    'color', 'baseWidth', 'seed', 'compressedTrace',
  ]
  ...
  toObject(additionalProperties: string[] = []) {
    ...
    for (let i = 0; i < this.crayonPoints.length; i++) {
      ...
      const ix = Math.round(p.x * 10), iy = Math.round(p.y * 10)
      if (i === 0) flat.push(ix, iy)
      else flat.push(ix - lastX, iy - lastY)
      lastX = ix; lastY = iy
    }
    const baseObj = super.toObject([
      'color', 'baseWidth', 'seed', ...additionalProperties,
    ] as any)
  ...
  static async fromObject(object: any) {
    ...
    if (!object.src) {
      ...
      const img = generateCrayonImage(pts, object.color, object.baseWidth, object.seed)
      if (img) {
        const props = await enlivenStrokeProps(object)
        return new CrayonStroke(img.getElement(), { ...props, crayonPoints: pts })
      }
    }
...
export class CrayonBrush extends PatternBrush {
  ...
  private _finalizeAndAddPath(): void {
    ...
    const img = generateCrayonImage(pts, this.color as string, this.width, this._seed)
    if (img) {
