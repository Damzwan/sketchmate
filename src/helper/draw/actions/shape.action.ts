import { Shape, ShapeCreationMode } from '@/types/draw.types'
import { useDrawStore } from '@/store/draw/draw.store'
import { EventBus } from '@/main'
import { storeToRefs } from 'pinia'
import { setSelectionForObjects } from '@/helper/draw/draw.helper'
import { exitClickShapeCreationMode, exitDragShapeCreationMode, findNearestPoint } from '@/helper/draw/shape.helper'
import { Canvas, Circle, Ellipse, FabricObject, Point, Line, Path, Polygon, Polyline, Rect, Triangle } from 'fabric'
...
export function addShape(options: any) {
  ...
  function enableShapeCreationClickEvents() {
    addEventsOfService('shapeCreation', [{
      on: 'mouse:down', handler: o => {
        ...
        let point: Point = { x: pointer.x, y: pointer.y }
        ...
        addToUndoStack({ type: 'polygonCreation', objects: [createdShape.toJSON()] })
...
function addShapeWithDrag(c: Canvas, shape: Shape) {
  ...
  addEventsOfService('shapeCreation', [{
    ...
    on: 'mouse:move', handler: o => {
      drawingMode ? handleMouseMove(c, shape, o, startX, startY, createdShape) : null
    }
  }, {
    on: 'mouse:up', handler: o => {
