...
export const useAPI = createGlobalState((): API => {
	...
	async function getUser(params: GetUserParams): Promise<Res<GetUserRes>> {
		try {
			...
			return await fetch(url, { method: REQUEST_TYPES.GET }).then((res) =>
				res.json(),
			);
		} catch (e) {
			console.log(e);
	...
	async function getPartialUsers(params: {
		_ids: string[];
	}): Promise<Res<Mate[]>> {
		try {
	...
	async function uploadProfileImg(
		...
		if (params.previousImage?.includes("stock"))
			params.previousImage = undefined;
	...
	async function getBalloon(params: {
		balloonId: string;
