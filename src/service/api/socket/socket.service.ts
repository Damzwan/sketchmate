...
export function createSocketService(): SocketAPI {
	...
	const { user, isLoading, isLoggedIn } = storeToRefs(useAuthStore());
	const { inbox } = storeToRefs(useInboxStore());
	const { friendRequestUsers, friendRequestLoading } = storeToRefs(
		useFriendStore(),
	);
	...
	async function connect(): Promise<void> {
		...
		socket.on(SOCKET_ENDPONTS.match, (params: Res<MatchRes>) => {
			...
			if (params.mate && params.mate._id != user.value?._id) {
				...
				user.value!.mate_requests_received =
					user.value!.mate_requests_received.filter(
						(m) => m != params.mate!._id,
					);
				user.value!.mate_requests_sent = user.value!.mate_requests_sent.filter(
					(m) => m != params.mate!._id,
				);
				toast(`Matched to ${params.mate.name}`, {
					buttons: [matchButton, dismissButton],
					duration: 5000,
				});
			} else {
				toast(`Failed to connect: ${params.error}`, {
					color: "danger",
		...
		socket.on(
			SOCKET_ENDPONTS.unmatch,
			async ({ unMatchedMateID, gotUnMatched }) => {
				if (!isLoggedIn.value) return;
				isLoading.value = false;

				const unMatchedMate = user.value!.mates.find(
					(u) => u._id == unMatchedMateID,
				);
				if (gotUnMatched) {
				...
				user.value!.mates = user.value!.mates.filter(
					(el) => el._id != unMatchedMate!._id,
				);
			},
		);
		...
		socket.on(SOCKET_ENDPONTS.send, async (params: Res<InboxItem>) => {
			...
			if (params) {
				...
				const followersNotInInboxUsers = params.original_followers.reduce(
					(acc: string[], curr) =>
						!inboxUsers.value.some((m) => m._id == curr) ? [...acc, curr] : acc,
					[],
				);
				...
				if (params.sender === user.value?._id) {
					...
					const sentDrawingsCount = inbox.value.reduce(
						(acc: number, curr) =>
		...
		socket.on(SOCKET_ENDPONTS.comment, (params: Res<CommentRes>) => {
			if (params) addComment(params);
		});

		socket.on(
			SOCKET_ENDPONTS.mate_request,
			async (params: SendMateRequestParams) => {
				friendRequestLoading.value = false;
				if (params.sender == user.value!._id) {
					toast("Request sent");
					...
					const newFriendRequestUser = (await getPartialUsers({
						_ids: [params.sender],
					})) as Mate[];
					friendRequestUsers.value = [
						...friendRequestUsers.value,
						...newFriendRequestUser,
					];
		...
		socket.on(
			SOCKET_ENDPONTS.cancel_mate_request,
			(params: SendMateRequestParams) => {
				friendRequestLoading.value = false;

				if (params.sender == user.value!._id) {
					toast("Friend request cancelled");
					user.value!.mate_requests_sent =
						user.value!.mate_requests_sent.filter((m) => m != params.receiver);
				} else {
					user.value!.mate_requests_received =
						user.value!.mate_requests_received.filter(
							(m) => m != params.sender,
		...
		socket.on(
			SOCKET_ENDPONTS.refuse_mate_request,
			(params: SendMateRequestParams) => {
				friendRequestLoading.value = false;
				if (params.sender == user.value!._id) {
					...
					user.value!.mate_requests_sent =
						user.value!.mate_requests_sent.filter((m) => m != params.sender);
				}
			},
		);
	}
	...
	async function cancelSendMateRequest(
		params: SendMateRequestParams,
	): Promise<void> {
		const { friendRequestLoading } = storeToRefs(useFriendStore());
		friendRequestLoading.value = true;
	...
	async function refuseSendMateRequest(
		params: SendMateRequestParams,
	): Promise<void> {
		const { friendRequestLoading } = storeToRefs(useFriendStore());
		friendRequestLoading.value = true;
