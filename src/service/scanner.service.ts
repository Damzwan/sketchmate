...
export function useScanner(videoRef: Ref<HTMLVideoElement | undefined> | undefined = undefined) {
  ...
  async function startScanning(): Promise<string | undefined> {
    if (isNative()) {
      ...
      if (status.camera == 'granted') {
        ...
        toast('Camera permission not granted or not available', { color: 'warning' })
        return undefined
      }
    } else {
