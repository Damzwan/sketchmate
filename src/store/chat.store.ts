...
export const useChatStore = defineStore("chat", () => {
	...
	function pruneMessageCaches(keep = MAX_CACHED_CONVERSATIONS) {
		...
		const candidates = ids
			...
			.sort(
				(a, b) =>
					(messageCacheAccess.get(a) || 0) - (messageCacheAccess.get(b) || 0),
			);
	...
	function addIncomingMessage(
		...
		if (
			...
			if (!chatWidget.isExpanded || chatWidget.activeTab !== conversation_id) {
	...
	async function loadMessages(
		...
		const existing = (messagesByChat.value[conversationId] || []).filter(
			(m) => !m.isOptimistic,
		);
	...
	async function switchToConversation(conversationId: string) {
		...
		touchMessageCache(conversationId);
		pruneMessageCaches();
		const realMessages = (messagesByChat.value[conversationId] ?? []).filter(
			(m) => !m.isOptimistic,
		);
		if (realMessages.length == 0) await loadMessages(conversationId, true);
