...
export function useShortcutManager() {
  ...
  async function handleKeydown(event: any) {
    ...
    if (event.key == Shortcut.delete2) {
      event.preventDefault()
      if (!isSelectMode.value) return
      selectAction(DrawAction.RemoveSelectedObjects, { objects: getSelectedObjects() })
      dismissPopover()
    }
    ...
    switch (event.key) {
      ...
      case Shortcut.flipX:
        event.preventDefault()
        if (!isSelectMode.value) return
        selectAction(DrawAction.FlipX, { objects: getSelectedObjects(), flipX: true })
        dismissPopover()
        break
      ...
      case Shortcut.flipY:
        event.preventDefault()
        if (!isSelectMode.value) return
        selectAction(DrawAction.FlipY, { objects: getSelectedObjects(), flipY: true })
        dismissPopover()
        break
      ...
      case Shortcut.paste:
        ...
        for (const item of items) {
          for (const type of item.types) {
            ...
            reader.onloadend = () => {
              selectAction(DrawAction.AddImage, { img: reader.result, ignoreResize: true })
            }
